...
with st.expander("ℹ️ How to Use This App", expanded=True):
    st.markdown("""
    **Welcome to Terrag AI!** This app helps you analyze business data with AI. Here's how it works:
...
def analyze_question(df, question, context="", sentence_model=None, qa_model=None):
    ...
    # Limit to top 1000 rows for semantic search
    ...
    top_row = sample_df.iloc[[top_index]]
...
# ---------- Automated Insights ----------
@st.cache_data
def generate_insights(df, qa_model=None):
    insights = []
...
if uploaded_file:
    ...
    try:
        if file_type == "csv":
            ...
            st.session_state.df = read_pdf(uploaded_file)
            if st.session_state.df is not None and len(st.session_state.df) > 10000:
                st.session_state.df = st.session_state.df.sample(10000, random_state=42)
        else:
            st.session_state.df = None
    except Exception as e:
